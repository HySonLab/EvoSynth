...
def tokens2smiles(tokens, reverse_alphabet, bos_token='[BOS]', eos_token='[EOS]'):
    """
    Convert token IDs to a SMILES string.
    - Skips the BOS token
    - Stops at EOS
    - Silently skips any unknown tokens
    """
    smiles = []
    for i, t in enumerate(tokens):
        # look up token safely
        tok = reverse_alphabet.get(int(t), None) if isinstance(reverse_alphabet, dict) else (
            reverse_alphabet[int(t)] if 0 <= int(t) < len(reverse_alphabet) else None
        )
        if tok is None:
            continue  # skip unknowns
        if i == 0 and tok == bos_token:
            continue  # skip BOS
        if tok == eos_token:
            break
        smiles.append(tok)
...
def padding_graph(arr: np.ndarray, padding_num: int = 64, dtype: np.dtype = ENCODING_DTYPE):
    ...
    if npad < 0:
        arr = arr[:padding_num, :padding_num] if arr.ndim == 2 else arr[:padding_num]
        npad = 0
    if arr.ndim < 2:
