...
def BBB_reward(smiles):
    predictions = admet_model.predict(smiles)
    return predictions['BBB_Martins'].tolist()

def toxicity_reward(smiles):
    predictions = admet_model.predict(smiles)
    return predictions['ClinTox'].tolist()
...
def dsdp_batch_reward(
    ...
    if gen_lig_pdbqt:
        ...
        for i in tqdm(range(smiles.shape[0])):
            try:
                ...
                print(f"[Error] Failed to convert SMILES {smi} to PDBQT: {e}")
                name_list.append(f'{i}.pdbqt')
    ...
    for i in tqdm(range(len(name_list))):
        ...
        try:
            ...
            print(f"[Error] Docking failed for ligand {i}.pdbqt: {e}")
            # Optionally write a dummy score file or log placeholder
            with open(log_dir, 'w') as f:
                f.write("Docking failed\n")
    ...
    for i in tqdm(range(len(name_list))):
        try:
            ...
            scores.append(0.0)  # clearly bad binding
    return scores