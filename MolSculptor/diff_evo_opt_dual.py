...
os.environ["CUDA_VISIBLE_DEVICES"] = "1"  # Set visible GPUs
import sys
...
from train.rewards import QED_reward, SA_reward, BBB_reward, toxicity_reward
...
def compute_alignment_loss(latents, scores, n_query_tokens):
    """
    Computes alignment loss as done during training.
    latents: np.array of shape (N, T, D) - full latent tokens
    scores: np.array of shape (N, M)
    n_query_tokens: int - number of prefix tokens used for pooling
    """

    # === 1. Get pooled latents from query tokens ===
    prefix_latents = latents[:, :n_query_tokens, :]  # (N, Q, D)
    pooled_latents = np.mean(prefix_latents, axis=1)  # (N, D)

    # === 2. Normalize pooled vectors ===
    norms = np.linalg.norm(pooled_latents, axis=-1, keepdims=True) + 1e-8
    pooled_latents = pooled_latents / norms  # (N, D)

    # === 3. Pairwise distance in latent space ===
    latent_dist = np.sum((pooled_latents[:, None, :] - pooled_latents[None, :, :]) ** 2, axis=-1)  # (N, N)

    # === 4. Pairwise distance in score space ===
    score_dist = np.sum((scores[:, None, :] - scores[None, :, :]) ** 2, axis=-1)  # (N, N)

    # === 5. Normalize score distances ===
    min_sd = np.min(score_dist)
    max_sd = np.max(score_dist)
    norm_score_dist = (score_dist - min_sd) / (max_sd - min_sd + 1e-8)

    # === 6. Compute alignment loss ===
    align_loss = np.mean((latent_dist - norm_score_dist) ** 2)
...
def infer(args):
    ...
    with open(args.vae_params_path, 'rb') as f:
        ...
        vae_params = jtu.tree_map(jnp.asarray, vae_params)
        encoder_params = vae_params['encoder']
        decoder_params = vae_params['decoder']

    with open(args.params_path, 'rb') as f:
        ft_params = pkl.load(f)
        ft_params = jtu.tree_map(jnp.asarray, ft_params)
    params = {'params': ft_params}
    # params = ft_params
   ...
   #### set inferencer
    ...
    def constraint_function(molecule_dict, cached, config):
        ...
        # Constraint 1: Substructure match
        sub_constraint = np.asarray(has_substructure(smiles, args.sub_smiles), np.int32)

        # Constraint 2: Uniqueness (no repeats)
        rep_constraint = find_repeats(smiles, unique_smiles)

        # Constraint 3: Similarity to initial scaffold
        sim = sim_function(smiles, template_smiles)
        sim_constraint = np.array(sim > config['sim_threshold'], np.int32)

        # Constraint 4: QED threshold
        qed = np.asarray(QED_reward(smiles), np.float32)
        qed_constraint = np.array(qed > config['qed_threshold'], np.int32)

        # Constraint 5: Synthetic Accessibility Score
        sas = np.asarray(SA_reward(smiles), np.float32)
        sas_constraint = np.array(sas < config['sas_threshold'], np.int32)

        # Constraint 6: Blood-Brain Barrier permeability
        # bbb = np.asarray(BBB_reward(smiles), np.float32)
        # bbb_constraint = np.array(bbb > config['bbb_threshold'], np.int32)
        
        #Constraint 7: Toxicity
        tox = np.asarray(toxicity_reward(smiles), np.float32)
        tox_constraint = np.array(tox < config['tox_threshold'], np.int32) 
    ...
    def diffusion_es_search_step(step_it, x, rng_key, config, cached):
        ...
        align_loss = compute_alignment_loss(np.array(x), np.array(scores), data_config['n_query_tokens'])
        cached['score_history'].append(scores)
        cached['latent_history'].append(x)
        recoder.info(f'[Step {step_it+1}] Alignment Loss: {align_loss:.6f}')
        ...
        # breakpoint() ## check here
        choiced_idx = NSGA_II(scores, constraints, config.constraint_weights, n_pops = DEVICE_BATCH_SIZE)
    ...
    def diffusion_es_optimization(config, rng_key, init_molecules,):
        ...
        init_constraints = np.stack([
            ...
            np.array([1] + [0] * (init_scores.shape[0] - 1), np.int32),             # rep
            np.ones((init_scores.shape[0],), np.int32),                             # sim
            np.ones((init_scores.shape[0],), np.int32),                             # qed
            np.ones((init_scores.shape[0],), np.int32),                             # sas
            # np.ones((init_scores.shape[0],), np.int32),                             # bbb
            np.ones((init_scores.shape[0],), np.int32),                             # tox
        ], axis=1)
    ...
    save_file = {
        ...
        'latent_history': cached['latent_history'],
