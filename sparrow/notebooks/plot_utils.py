...
def graph_vis(tree_path: str, routes_path: str, cleaned_tar_path: str, percent_plot=0.1): 
    ...
    nx.draw_networkx_edges(G, pos, edgelist=G.edges(), edge_color='lightgray',
                           node_size=ns, width=0.5, arrowsize=5, ax=ax1)
    nx.draw_networkx_nodes(G, pos, nodelist=G.nodes(), node_color='lightgray',
                           node_size=ns, ax=ax1, edgecolors='lightgray', linewidths=0.1)
    
    nx.draw_networkx_edges(G, pos, edgelist=edgelist2, edge_color='dimgrey',
                           node_size=ns, width=0.5, arrowsize=5, ax=ax1)
    nx.draw_networkx_nodes(G, pos, nodelist=nodelist2, node_color=nodecolor,
                           node_size=ns, ax=ax1, edgecolors='k', linewidths=0.1)
    
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', label='Target',
               markerfacecolor=theme_colors[2], markersize=8),
        Line2D([0], [0], marker='o', color='w', label='Intermediate',
               markerfacecolor=theme_colors[3], markersize=8),
        Line2D([0], [0], marker='o', color='w', label='Starting Material',
               markerfacecolor=theme_colors[0], markersize=8),
        Line2D([0], [0], marker='o', color='w', label='Reaction',
               markerfacecolor=theme_colors[1], markersize=8),
    ]
...
def graph_vis_ax(
    ...
    tree_path: str,
    routes_path: str,
    cleaned_tar_path: str,
    percent_plot: float = 0.10,
    title: str | None = None,
):
    ...
    sel_rxns = {item['smiles'] for entry in routes.values() for item in entry['Reactions']}
    sel_tars = list(routes.keys())
    sel_cpds = set([*sel_tars, *[item for entry in routes.values() for item in entry['Compounds']]])
    ...
    # ---- layout + draw on provided ax ----
    pos = nx.spring_layout(G)

    nodelist_focus = [n for n in G.nodes() if n in sel_ids]
    nodecolor = []
    for n in nodelist_focus:
        if n in tar_ids:
            nodecolor.append(theme_colors[2])  # Target
        elif n in cpd_ids:
            nodecolor.append(theme_colors[3])  # Intermediate/candidate
        elif n in start_ids:
            nodecolor.append(theme_colors[0])  # Starting material (dummy rxns)
        else:
            nodecolor.append(theme_colors[1])  # Reaction

    edgelist_focus = [e for e in G.edges() if e in sel_edges]
    ns = 20
    nx.draw_networkx_edges(G, pos, edgelist=G.edges(), edge_color='lightgray',
                           node_size=ns, width=0.5, arrowsize=5, ax=ax)
    nx.draw_networkx_nodes(G, pos, nodelist=G.nodes(), node_color='lightgray',
                           node_size=ns, ax=ax, edgecolors='lightgray', linewidths=0.1)
    nx.draw_networkx_edges(G, pos, edgelist=edgelist_focus, edge_color='dimgrey',
                           node_size=ns, width=0.5, arrowsize=5, ax=ax)
    nx.draw_networkx_nodes(G, pos, nodelist=nodelist_focus, node_color=nodecolor,
                           node_size=ns, ax=ax, edgecolors='k', linewidths=0.1)

    ax.set_axis_off()
    if title:
        ax.set_title(title, fontsize=11, pad=6)

    # return handles once so we can build a shared legend
    legend_elems = [
        Line2D([0], [0], marker='o', color='w', label='Target',
               markerfacecolor=theme_colors[2], markersize=8),
        Line2D([0], [0], marker='o', color='w', label='Intermediate',
...
def render_retrosynthesis_grid(
    cfg_ad: dict,   # keys: 'molsculptor', 'cmdgen', 'contrastive', 'predictor'
    cfg_oc: dict,   # same structure as cfg_ad
    save_path: str = "figures/retrosynthesis_grid.pdf",
    percent_plot: float = 0.10,
):
    """
    Render a 2x4 grid of retrosynthesis graphs:
      rows  : AD (top), OC (bottom)
      cols  : MolSculptor | CMD-GEN | EvoSynth (Contrastive) | EvoSynth (Predictor)
    Places the case labels as vertical row labels on the left (no overlap with subplot titles).
    """
    titles_row = [
        "EvoSynth (Contrastive)",
        "EvoSynth (Predictor)",
        "MolSculptor",
        "CMD-GEN",
    ]

    fig, axes = plt.subplots(2, 4, figsize=(14, 6.6), dpi=600)
    ...
    # ---- Vertical row labels ----
    fig.text(0.015, 0.75, "AD (JNK3/GSK3β)",
             va="center", ha="left", fontsize=12, fontweight="bold", rotation=90)
    fig.text(0.015, 0.25, "OC (PI3K/PARP)",
             va="center", ha="left", fontsize=12, fontweight="bold", rotation=90)
    ...
    if handles is not None:
        fig.legend(
            handles,
            ["Target", "Intermediate", "Starting Material", "Reaction"],
            loc="lower center",
            ncol=4,
            frameon=False,
            borderaxespad=0.5,
    ...
    # Adjust layout for 4 columns
    plt.subplots_adjust(
        left=0.07, right=0.98, top=0.93, bottom=0.12,
        wspace=0.10, hspace=0.20
    )

    fig.savefig(save_path, bbox_inches="tight", dpi=600)
    print(f"Saved 2×4 retrosynthesis grid to {save_path}")
    return fig
